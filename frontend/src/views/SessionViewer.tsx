...
export default function SessionViewer(props: SessionViewerProps) {
  const [session] = createResource(() => props.sessionId, apiClient.getSession);
  ...
  createEffect(() => {
    if (initialized() || session.loading || !session()) return;
    setInitialized(true);
  ...
  const exportTranscript = (format: "json" | "markdown") => {
    const data = messages();
    if (format === "json") {
      downloadFile(`${props.sessionId}-transcript.json`, JSON.stringify(data, null, 2));
      return;
    }
    const markdown = data
  ...
  return (
    <div class="session-viewer">
            ...
            <div class="guardrail-helper" role="note">
              ...
              <p class="guardrail-helper-text">
                {guardrailDetail("bulk-operations") ||
                  "Bulk session controls are restricted by current guardrails."}
              </p>
      ...
      <Show when={!permissions.loading && !canInspect()}>
        <p class="guardrail-banner error">
          <span>
